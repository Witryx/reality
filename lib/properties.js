...
export async function fetchProperties(language) {
  ...
  if (language) {
    const { rows } = await sql`
      SELECT id, name, location, price, sqm, rooms, sort_order AS "sortOrder", image, images, videos, tag, description, sold, language, created_at
      FROM properties
...
export async function createProperty(input) {
  ...
  try {
    ...
    const { rows } = await sql`
      INSERT INTO properties (name, location, price, sqm, rooms, sort_order, image, images, videos, tag, description, language)
      VALUES (${payload.name}, ${payload.location}, ${payload.price}, ${payload.sqm}, ${payload.rooms}, ${payload.sortOrder}, ${payload.image}, ${imagesJson}::jsonb, ${videosJson}::jsonb, ${payload.tag}, ${payload.description}, ${payload.language})
      RETURNING id, name, location, price, sqm, rooms, sort_order AS "sortOrder", image, images, videos, tag, description, sold, language, created_at;
    `;
...
export async function updateProperty(input = {}) {
  ...
  const { rows: existingRows } = await sql`
    SELECT id, name, location, price, sqm, rooms, sort_order AS "sortOrder", image, images, videos, tag, description, sold, language
    FROM properties
    WHERE id = ${propertyId}
    LIMIT 1;
  `;
